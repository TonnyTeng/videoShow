...
- (instancetype)initWithFrame:(CGRect)frame Image:(NSString *)imageName centerIcon:(NSString *)icon
{
    ...
    self.nameLabel.text = @"iOS技术交流群：528243317";
        ...
        self.wavePeriod = 1;
        self.waveLength = ALinScreenWidth;
    }
    return self;
}
...
- (void)setShapeLayerPath {
    ...
    self.offsetX += (ALinScreenWidth / 60 / self.wavePeriod);
...
- (UIBezierPath *)currentWavePath {
    ...
    for (float x = 0.0f; x <= ALinScreenWidth * 2; x++) {
    ...
    CGPathAddLineToPoint(path, nil, ALinScreenWidth, self.height);
